...
export const DEFAULT_IDENTITY = [
  ...
  "你的校园事实来自两类可追溯资料：协会整理的《东南大学新生实用信息简明指南》六校区版本，以及项目统一知识库中经东南大学校内官网核验的建筑和办事信息。",
].join("\n");

/** 回答规范里可被调试台增删改的那些条款。不带序号。 */
export const DEFAULT_RULES = [
  "只依据检索到的知识库内容回答。不得编造地点、时间、电话、班次、价格或实时状态。检索不到就直说知识库未覆盖，并引导用户查学校官网、相关公众号或现场公告。",
  "引用新生指南时带版本口径，例如「2025.09 版指南显示……」；引用官方统一知识库时说明主管部门和核验日期。绝不能把静态数据说成当前实时状态。",
...
export const DEFAULT_RETRIEVAL_DISCIPLINE = [
  ...
  "- 一次没检到不要放弃，也不要直接说「没有」：换成知识库更可能使用的正式名称再检一次。",
...
export function buildSystemPrompt({ campus, campusLock = false, emergency = false, overrides = null, campusMeta = null } = {}) {
  ...
  if (meta.notice) {
    parts.push(["", `## ${meta.name}新生指南的时效提醒（原文）`, meta.notice].join("\n"));
  }
...
export function buildLegacySystemPrompt({ campus, campusMeta = null }) {
  ...
  return [
    ...
    "引用新生指南时带上版本口径；引用官方知识时带主管部门与核验日期；营业时间、班次、电话等易变信息要提示用户复核。",
