// 调试台的口令登录与会话 Cookie。
//
// 与 web/api/_shared/admin-auth.js 同源，只改了 Cookie 名——两个项目部署在不同
// 域名下，各自持有独立口令，地图后台的口令泄露不应该顺带打开 agent 调试台。
//
// 口令本身不落盘也不比较明文：两边都先用 HMAC 签一次再定长比较，避免时序泄露。

const cookieName = "seu_agent_admin";
...
/**
 * 本地开发走 http://127.0.0.1，而 Safari 不接受 http 下的 Secure Cookie，
 * 带上就会登录不进去。所以本地开发去掉 Secure，其余一切情况都必须带。
 *
 * 判据用 process.env.VERCEL 而不是请求里的 Host——Host 头是客户端可控的，
 * 拿它决定要不要降级安全属性等于把开关交给攻击者。Vercel 上这个变量恒为 "1"。
 */
function cookieAttributes(request) {
  ...
  if (!onVercel) {
    try {
      ...
      localHttp = url.protocol === "http:" && ["localhost", "127.0.0.1", "[::1]"].includes(url.hostname);
    } catch {
      localHttp = false;
    }
  }
  return `Path=/; HttpOnly; ${localHttp ? "" : "Secure; "}SameSite=Strict`;
}
...
export async function createSessionCookie(request) {
  ...
  return `${cookieName}=${token}; ${cookieAttributes(request)}; Max-Age=${sessionLifetimeSeconds}`;
}

export function clearSessionCookie(request) {
  return `${cookieName}=; ${cookieAttributes(request)}; Max-Age=0`;
}
